    ...
    static public void removeColliderAndApplyPrefabChanges()
    {
        ...
        if (obj != null)
        {
            for (int i = 0; i < obj.Length; i++)
            {
                ...
                var prefab_src = PrefabUtility.GetCorrespondingObjectFromOriginalSource(prefab_root);
                if (prefab_src != null)
                {
                    ...
                    {
                        // apply updated
                        PrefabUtility.SaveAsPrefabAssetAndConnect(prefab_root, AssetDatabase.GetAssetPath(prefab_src), InteractionMode.AutomatedAction);
